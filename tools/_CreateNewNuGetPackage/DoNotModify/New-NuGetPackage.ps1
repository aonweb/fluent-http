...
$NuPkgOutputPath = $ProjectPath + "\bin\Release"
...
try
{
	...
	if ((Test-StringIsNullOrWhitespace $NuSpecFilePath) -and (Test-StringIsNullOrWhitespace $ProjectFilePath) -and (Test-StringIsNullOrWhitespace $PackageFilePath))
	{
		...
		$nuSpecFiles = Get-ChildItem "$ProjectPath\*" -Include "*.nuspec" -Name
		
		# Get all of the project files in the script's directory.
		$projectFiles = Get-ChildItem "$ProjectPath\*" -Include $VALID_NUGET_PROJECT_TYPE_EXTENSIONS_WITH_WILDCARD_ARRAY -Name

        # Get all of the NuGet package files in this script's directory.
        $packageFiles = Get-ChildItem "$ProjectPath\*" -Include "*.nupkg" -Name
		...
		{
			$firstProjectPath = Join-Path $ProjectPath ($projectFiles | Select-Object -First 1)
			$projectsNuSpecFilePath = Get-ProjectsAssociatedNuSpecFilePath -ProjectFilePath $firstProjectPath
			
			# If we didn't find any .nuspec files, then use this project file.
			if ($numberOfNuSpecFilesFound -eq 0)
			{
				$ProjectFilePath = $firstProjectPath
			}
			...
			elseif ($numberOfNuSpecFilesFound -eq 1)
			{
				...
				$nuSpecFilePathInThisScriptsDirectory = Join-Path $ProjectPath ($nuSpecFiles | Select-Object -First 1)
		...
		{
			$NuSpecFilePath = Join-Path $ProjectPath ($nuSpecFiles | Select-Object -First 1)
		}
        ...
        {
            $PackageFilePath = Join-Path $ProjectPath ($packageFiles | Select-Object -First 1)
        }
		...
		if ((Test-StringIsNullOrWhitespace $NuSpecFilePath) -and (Test-StringIsNullOrWhitespace $ProjectFilePath) -and (Test-StringIsNullOrWhitespace $PackageFilePath))
		{
			...
			if ([System.IO.Path]::GetExtension($filePathToUse) -eq ".nuspec")
			{
				...
				$firstProjectPath = Get-NuSpecsAssociatedProjectFilePath -NuSpecFilePath $filePathToUse
				if (!(Test-StringIsNullOrWhitespace $firstProjectPath))
				{
                    ...
                    {
					    $promptMessage = "The selected .nuspec file appears to be associated with the project file:`n`n$firstProjectPath`n`nIt is generally preferred to pack the project file, and the .nuspec file will automatically get picked up.`nDo you want to pack the project file instead?"
